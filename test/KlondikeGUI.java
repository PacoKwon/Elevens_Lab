...
public class KlondikeGUI extends JFrame implements ActionListener {
     ...
     */
    public KlondikeGUI(Board gameBoard) {
        board = gameBoard;
        totalWins = 0;
        totalGames = 0;
        ...
        public void mouseClicked(MouseEvent e) {
            for (int k = 0; k < board.size(); k++) {
                if (e.getSource().equals(displayCards[k]) && board.cardAt(k) != null) {
                    ...
                    System.out.printf("Card Number %d %s %s\n", k, board.cardAt(k).suit(), board.cardAt(k).rank());
                    repaint();
                    return;
                }
            }
